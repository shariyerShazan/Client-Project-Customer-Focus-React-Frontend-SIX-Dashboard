...
// import { Executive } from "./SuperAdminMenu";
import logo from "@/assets/superAdmin/E2Elogo.svg";
// import { MdExitToApp } from "react-icons/md";
// import { IoExitOutline } from "react-icons/io5";
import { LuLogOut } from "react-icons/lu";
import { ExecutiveMenuItems } from "./ExecutiveMenu";
...
export function ExecutiveSidebar() {
  ...
  const findParentItem = (childId: string): string | null => {
    for (const item of ExecutiveMenuItems) {
  ...
  const renderMenuItems = (
    items: typeof ExecutiveMenuItems,
    level = 0
  ) =>
    items.map((item) => {
    ...
    <>
      ...
      <nav className="flex flex-col h-[80vh]">
        <div className="mt-6 space-y-1">
            {renderMenuItems(ExecutiveMenuItems)}
        </div>
